...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    T oldVal = _elems[idx + 1];
    _elems[idx + 1] = elem;

  	size_t parentIdx = parent(idx + 1);
    size_t priorityChildIdx = maxPriorityChild(idx + 1);

  	if (higherPriority(_elems[idx + 1], _elems[parentIdx]))
        heapifyUp(idx + 1);
  	if (higherPriority(_elems[priorityChildIdx], _elems[idx + 1]))
        heapifyDown(idx + 1);
}
