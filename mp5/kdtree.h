...
//     bool biggerraduis(Point<Dim> target,Point<Dim> subroot,int d,double radius) const;
//     double getradius(Point<Dim> first,Point<Dim>second) const;
// };
//
// #include "kdtree.hpp"
...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    void treebuilder(vector<Point<Dim>>& p,KDTreeNode* &subroot,int d,int idx1,int idx2);
    int constructionhelper(int idx1,int idx2,vector<Point<Dim>>& newPoints,int d,int mid);
    void destroy_(KDTreeNode * subroot);//Destructor helper function, takes in a subroot and free the subtree
    /** Helper function for grading */
    int getPrintData(KDTreeNode * subroot) const;
    Point<Dim> findnhelper(KDTreeNode * subroot,int d,Point<Dim>query) const;
    bool biggerraduis(Point<Dim> target,Point<Dim> subroot,int d,double radius) const;
    /** Helper function for grading */
    void printTree(KDTreeNode * subroot, std::vector<std::string>& output,
                   int left, int top, int width, int currd) const;
